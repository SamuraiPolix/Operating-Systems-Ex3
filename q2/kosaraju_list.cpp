...
KosarajuList::KosarajuList(int vertices) : n(vertices) {
    adj.resize(n);
    std::cout << "Initialized KosarajuList with " << vertices << " vertices.\n";
}

void KosarajuList::addArc(int u, int v) {
    if (u < 1 || u > n || v < 1 || v > n) {
...
std::vector<std::list<int>> KosarajuList::transposeGraph() {
    std::vector<std::list<int>> transposed(n);
    for (int v = 0; v < n; ++v) {
        for (int i : adj[v]) {
...
void KosarajuList::dfs(int v, std::vector<std::list<int>> &adj, std::vector<bool> &visited, std::stack<int> &Stack) {
    visited[v] = true;
    for (int i : adj[v])
        if (!visited[i])
...
void KosarajuList::dfsOnTranspose(int v, std::vector<std::list<int>> &adj, std::vector<bool> &visited, std::vector<int> &scc) {
    visited[v] = true;
    scc.push_back(v);
    for (int i : adj[v])
        if (!visited[i])
...
void KosarajuList::findSCCs() {
    ...
    // Transpose the graph
    std::vector<std::list<int>> transposed = transposeGraph();
